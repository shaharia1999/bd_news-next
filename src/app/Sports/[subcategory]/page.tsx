...
export default async function SubCategoryPage({
  ...
  const validSubs = subCategoriesMap['Sports'].map((s) => s.toLowerCase());
...
export async function generateMetadata({ params }: { params: Promise<PageProps> }) {
  ...
  const title = `${subcategory.charAt(0).toUpperCase() + subcategory.slice(1)} News & Updates - Sports | NewsUS`;
  const description = `Stay updated with the latest ${subcategory} news, sports scores, highlights, and trending stories. Get curated sports content and insights in the Sports category at NewsUS.`;
  ...
  return {
    ...
    openGraph: {
      title,
      description,
      url: `${siteUrl}/Sports/${subcategory}`,
      type: 'website',
      images: [
        { url: image, width: 1200, height: 630, alt: `${subcategory} sports news and updates` },
      ],
    },
    twitter: {
    ...
    alternates: {
      canonical: `${siteUrl}/Sports/${subcategory}`,
    },
  };
}
