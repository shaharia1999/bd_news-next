...
export default async function SubCategoryPage({ params }: { params: Promise<PageProps> }) {
  ...
  return (
    <div className="px-3 pb-10 w-full">
      <h1 className="text-2xl font-bold capitalize my-5">{subcategory} News</h1>
...
export async function generateMetadata({ params }: { params: Promise<PageProps> }) {
  ...
  const title = `${subcategory.charAt(0).toUpperCase() + subcategory.slice(1)} News & Updates - Politics | NewsUS`;
  const description = `Stay informed with the latest ${subcategory} news, political updates, and analysis from around the world. Get curated stories and insights in the Politics category at NewsUS.`;
  ...
  return {
    ...
    openGraph: {
      title,
      description,
      url: `${siteUrl}/Politics/${subcategory}`,
      type: 'website',
      images: [
        { url: image, width: 1200, height: 630, alt: `${subcategory} political news and updates` },
      ],
    },
    twitter: {
    ...
    alternates: {
      canonical: `${siteUrl}/Politics/${subcategory}`,
    },
  };
}
