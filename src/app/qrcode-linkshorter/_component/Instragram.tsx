...
import { LuInstagram, LuDownload } from "react-icons/lu";
...
const InstagramDownloader = () => {
  ...
  // Redirect to SaveFrom.net with Instagram URL
  const handleDownload = () => {
    if (!videoUrl) {
      setMessage('Please enter Instagram URL');
      return;
    }
    ...
    // Pre-fill SaveFrom.net input
    const redirectUrl = `https://en.savefrom.net/18/#url=${encodeURIComponent(videoUrl)}`;
    ...
    <div className="flex justify-center items-center min-h-screen bg-gray-100 p-4">
      <div className="relative w-full max-w-lg mx-auto bg-white rounded-3xl shadow-xl p-8 space-y-6 transform transition-all duration-300 hover:scale-105">
        <div className="relative flex items-center bg-gray-50 rounded-full p-2 shadow-inner">
          ...
          <input
            type="text"
            className="flex-grow p-2 pl-4 bg-transparent outline-none text-gray-700 bg-white"
        ...
        <div className="flex flex-col items-center space-y-4">
          ...
          <button
            ...
            className="w-full flex justify-center items-center space-x-2 px-6 py-3 bg-green-500 text-white font-bold rounded-full shadow-lg transition-colors duration-200 hover:bg-green-600"
          >
